...
# from Scripts.fastapp import models
from Scripts.fastapp.common.consts import JWT_SECRET, JWT_ALGORITHM
from Scripts.fastapp.database.conn import db
from Scripts.fastapp.database.schema import Users
from Scripts.fastapp.models import SnsType, Token, UserToken, UserRegister
...
async def register(sns_type: SnsType, reg_info: UserRegister, session: Session = Depends(db.session)):
    ...
    if sns_type == SnsType.email:
        ...
        # pw.decode 해야함. 그래야 뒤에 로그인 할때 DB에서 불러와서 정상적으로 비교해 볼 수 있음.
        # decode( )를 하지 않고 DB에 입력되었던 경우, 로그인 단계에서 password를 호출했을 때 type=str이어서 다시 byte로 바꾸어주어야했다.
